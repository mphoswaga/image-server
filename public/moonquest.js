...
(() => {
  ...
  function diagramHtml(d, selection, accepted = [], edit = false) {
    ...
    return `<div class="diagram" id="diagram"><img src="${imageUrl(d.asset)}" alt="${esc(d.title || 'Lesson diagram')}" draggable="false"><svg viewBox="0 0 1 1" preserveAspectRatio="none" aria-label="${edit?'Draw answer areas':'Choose an answer area'}">${d.regions.map((r, i) => `<polygon data-region="${esc(r.id)}" points="${r.points.map(p=>p.join(',')).join(' ')}" class="${selectedIds(selection).includes(r.id)?'selected ':''}${accepted.includes(r.id)?'correct':''}" tabindex="${edit?'-1':'0'}" role="button" aria-label="${esc(r.label)}" aria-pressed="${selectedIds(selection).includes(r.id)}"><title>${i+1}. ${esc(r.label)}</title></polygon>`).join('')}<polygon class="draft" id="drawing" points=""></polygon></svg></div>`;
  }
  ...
  function presenterControls(s) {
    ...
    return `<div class="teacher-transport control presenter-controls" aria-label="Presentation controls">${s.phase==='lobby'?button('Begin mission','launch','primary'):''}${s.phase==='intro'&&!s.paused?button('Skip intro','skip-intro'):''}${!['lobby','ended'].includes(s.phase)?button(s.paused&&s.teachingPause==='misconception'&&s.singleTimer?'Continue after discussion':s.paused?'Resume':'Pause',s.paused&&s.teachingPause==='misconception'&&s.singleTimer?'continue-meeting':'pause'):''}${s.deadline&&s.phase!=='intro'?button('+10 seconds','extend'):''}</div>`;
  }
