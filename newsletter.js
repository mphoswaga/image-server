...
async function generateNewsletter(source, timing = 'next', ai = client()) {
  const response = await ai.chat.completions.create({
    ...
    response_format: { type: 'json_schema', json_schema: { name: 'parent_newsletter', strict: true, schema: { type: 'object', additionalProperties: false, properties: { learning: { type: 'string' }, homeSupport: { type: 'string' }, vocabulary: { type: 'array', minItems: 1, maxItems: 6, items: { type: 'object', additionalProperties: false, properties: { term: { type: 'string' }, meaning: { type: 'string' } }, required: ['term', 'meaning'] } } }, required: ['learning', 'homeSupport', 'vocabulary'] } } },
  });
  const result = JSON.parse(response.choices[0]?.message?.content || '{}');
  if (typeof result.learning !== 'string' || !result.learning.trim() || typeof result.homeSupport !== 'string' || !result.homeSupport.trim() || !Array.isArray(result.vocabulary) || result.vocabulary.length < 1 || result.vocabulary.length > 6 || result.vocabulary.some(item => !item || typeof item.term !== 'string' || !item.term.trim() || typeof item.meaning !== 'string' || !item.meaning.trim())) throw new Error('The newsletter was incomplete. Please try again.');
  const vocabulary = result.vocabulary.map(item => `${item.term.trim()}: ${item.meaning.trim()}`).join('\n');
